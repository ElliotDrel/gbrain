...
describe('#1433 — isSyncable / unsyncableReason are duals of one classifier', () => {
  const cases: Array<{ path: string; expected: SyncableReason | null; note: string }> = [
    ...
    { path: 'RESOLVER.md', expected: 'metafile', note: 'top-level master routing config (B7)' },
    { path: 'brain/RESOLVER.md', expected: 'metafile', note: 'RESOLVER.md anywhere is metafile (B7)' },
