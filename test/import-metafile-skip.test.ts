/**
 * Fork patch B6 (2026-06-15): bulk-import walker must skip SYNC_SKIP_FILES
 * metafiles (README.md / index.md / log.md / schema.md), the same way
 * incremental `sync` (isSyncable) does.
 *
 * Root cause this locks: the PGLite→Supabase migration's directory-restore
 * pass imported every directory README.md as a page (titled "People",
 * "Companies", …), because collectSyncableFiles only filtered by extension.
 * Those index-titled pages then trigram-corrupted fuzzy entity resolution.
 */
...
describe('collectSyncableFiles metafile exclusion (B6)', () => {
  function seed(): void {
    write('people/elliot-drel.md', '# Elliot Drel\n');
