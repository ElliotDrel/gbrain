...
| Exit `4`, raw persisted with `_transcript_state: empty-error` | **Stop.** Relay the exact fetch/provider failure. This is transient unknown, not proven no-speech, so do **not** file a transcript-less page from it. |
