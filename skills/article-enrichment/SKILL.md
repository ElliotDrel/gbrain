...
#    Mark NEW PEOPLE pages minted for article subjects the reader only reads
#    ABOUT (not personal contacts) as reference:
#    `gbrain reference <people/slug>` (or reference: true in frontmatter).
#    Exempts them from coverage nudges; they stay searchable. Companies are
#    never reference. Default for real contacts: do NOT set it. Hard stop: if
#    the reference decision is unclear, read
#    `skills/conventions/reference-entities.md` before creating the page.
```
